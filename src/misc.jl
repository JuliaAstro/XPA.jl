...
yields the value associated with configuration parameter `key`.  The following
parameters are available (see XPA doc. for more information):

* `"XPA_MAXHOSTS"` (default value is `100`).

* `"XPA_SHORT_TIMEOUT"` (default value is `15`).

* `"XPA_LONG_TIMEOUT"` (default value is `180`).

* `"XPA_CONNECT_TIMEOUT"` (default value is `10`).

* `"XPA_TMPDIR"` (default value is `"/tmp/.xpa"`).

* `"XPA_VERBOSITY"` (default value is `true`).

* `"XPA_IOCALLSXPA"` (default value is `false`).
...
_free(ptr::Ptr{T}) where T =
    (ptr == convert(Ptr{T}, 0) || ccall(:free, Void, (Ptr{T},), ptr))
...
_memcpy!(dst::Ptr, src::Ptr, len::Integer) :: Ptr{Byte} =
    ccall(:memcpy, Ptr{Byte}, (Ptr{Void}, Ptr{Void}, Csize_t), dst, src, len)
